...
- **Discord adapter** — implemented (DM-only); live WS round-trip is operator-verified. Slack/Mattermost/Matrix adapters — not yet implemented (framework ready: adapter registry + `CredSpec` + render subsystem all support a new platform via `init()` registration alone). The connectors UI (`/dashboard/connectors`) is now `CredSpec`-driven — a new platform's connect card is data, not hand-written template markup.
