...
func TestExecuteBuildBlocksMutating(t *testing.T) {
	...
	_, err := Execute(context.Background(), reg, fakeStore{tok: "AT"}, http.DefaultClient,
		...
		map[string]any{"to": "a@b.com", "body": "hi"}, Policy{BuildPhase: true}) // buildPhase = true
...
func TestExecuteBadArgs(t *testing.T) {
	reg := testRegistry(t)
	_, err := Execute(context.Background(), reg, fakeStore{tok: "AT"}, http.DefaultClient,
		ConnRef{Provider: "google"}, "gmail_search", map[string]any{}, Policy{}) // missing query
...
func TestExecuteMapsProviderError(t *testing.T) {
	...
	_, err := Execute(context.Background(), reg, fakeStore{tok: "AT"}, srv.Client(),
		ConnRef{Provider: "google"}, "gmail_search", map[string]any{"query": "x"}, Policy{})
	if ce, ok := err.(*ConnectorError); !ok || ce.Kind != KindAuth {
		t.Fatalf("expected KindAuth, got %v", err)
	}
}

// The zero Policy must be the permissive default: no build guard, no gate. Runs,
// chat turns, and the livecheck harness all pass Policy{}, and a future field
// that defaulted to restrictive would silently break every one of them.
func TestZeroPolicyIsPermissive(t *testing.T) {
	var p Policy
	if p.BuildPhase {
		t.Error("zero Policy must not enable the build guard")
	}
}
