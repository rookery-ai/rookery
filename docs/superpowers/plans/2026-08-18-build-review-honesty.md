...
Two smaller constraints for the executor: pass the SAME `ctx`, so `Cancel()` still stops a dry run mid-flight — never `context.Background()`; and place the call after `decideBuildOutcome` but before `cleanupOnSuccess()` at `flow.go:1906`, since the dry run needs the working directory to still exist.
